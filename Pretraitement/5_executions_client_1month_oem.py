...
@transform_df(
    Output('/ABM-f23a10/ABM - Revue des modèles/2025/output/FV/S07_DCCIV/executions_client_1month'),
...
#    df_exec_client = df_exec_client.filter(df_exec_client["date_traitement"] == date)
    ...
    df_exec_client = df_exec_client.withColumn("month_to_study",
    ...
    df_exec_client = df_exec_client.filter(F.add_months(F.col("date_ouverture"), param2) <= F.col("end_time"))

    df_exec_client = df_exec_client.filter((F.col("start_date_ts") == F.col("start_time")) & (F.col("end_date_ts") == F.col("end_time")))
